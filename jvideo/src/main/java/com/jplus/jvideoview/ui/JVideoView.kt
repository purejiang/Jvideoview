...
//                    mPresenter?.pausePlay()
        ...
        ly_video_center.setOnTouchListener { v, event ->
            when (event.action) {
                ...
                MotionEvent.ACTION_UP -> {
                    ...
                    if (abs(event.rawX - mStartX) > 80||abs(event.rawY - mStartY)> 80) {
                        when (mAdjustWay){
                            JVideoState.PlayAdjust.ADJUST_VOLUME->{
                                //音量调节
                ...
                MotionEvent.ACTION_MOVE ->{
                    Log.d("pipa", "action_move:X=" + event.rawX + ",y:" + event.rawY)
                        when (mAdjustWay){
                            JVideoState.PlayAdjust.ADJUST_VOLUME->{
                                //音量调节
                                mPresenter?.setVolume(event.rawY - mStartY)
                            }
                            JVideoState.PlayAdjust.ADJUST_LIGHT-> {
                                // 亮度调节
                                ...
                                //快进/后退
                                mPresenter?.forwardOrBackVideo(event.rawX - mStartX)
                            }
                        }
    ...
    override fun preparedPlay() {
        showLoading(false)
        tv_video_playing_progress.text = JVideoUtil.progress2Time(null) + "/" + JVideoUtil.progress2Time(mPresenter?.getDuration())
    ...
    override fun seekToPlay(position: Int) {
        tv_video_playing_progress.text = JVideoUtil.progress2Time(position) + "/" + JVideoUtil.progress2Time(mPresenter?.getDuration())
        seek_video_progress?.progress = position
    }
